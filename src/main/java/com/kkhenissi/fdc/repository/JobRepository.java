...
import com.kkhenissi.fdc.domain.Job;
...
public interface JobRepository extends JpaRepository<Job, Long> {
    ...
    @Query(value = "select distinct job from Job job left join fetch job.items",
        countQuery = "select count(distinct job) from Job job")
    Page<Job> findAllWithEagerRelationships(Pageable pageable);

    @Query("select distinct job from Job job left join fetch job.items")
    List<Job> findAllWithEagerRelationships();

    @Query("select job from Job job left join fetch job.items where job.id =:id")
